...
class FacebookPageService
{
    ...
    public static function postToPage($page, $content, $imgLink, $shortenLink, $directoryPath)
    {
        ...
        foreach ($imgLink as $i => $row) {
            ...
            $postImage = null;
            $linkLastPart = str_replace("-", " ", $linkLastPart);
            ...
            $postContent = ($content ? $content . ' ' : '') . $shortenLink[$i - 1]['link'];
            try {
                $postId = self::tryPost($page['id'], $postContent, $postImage, $page['access_token']);

                $shortenLink[$i - 1]['id'] = $postId;
            } catch (\Exception $e) {
                $shortenLink[$i - 1]['id'] = 0;
                $shortenLink[$i - 1]['error'] = $e->getMessage();
            }
        }
        return $shortenLink;
    }
