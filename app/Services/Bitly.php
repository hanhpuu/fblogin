...
class Bitly
{
    ...
    public static function getShortenLinkByFbIds($postIds, $token) {
        $client = new \GuzzleHttp\Client();
        $result = [];
        foreach ($postIds as $postId) {
            $url = 'https://facebook.com/' . $postId;
            try {
                $result[$postId] = ['link' => self::tryShortenLink($client, $url, $token)];
            } catch(\Exception $e) {
                $result[$postId] = ['link' => null, 'error' => $e->getMessage()];
            }
        }
        return $result;
    }
